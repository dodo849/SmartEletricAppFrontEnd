  ...
  Future<Result<bool, String>> execute(
    ...
    if (getBillSimulationProductResult.status == ResultStatus.error) {
      return Result.failure(
          "Failed to import bill simulation products from storage");
    }
    ...
    if (getBillSimulationProductResult.value != null) {
      ...
      List<BillSimulationProductModel> billSimulationProducts =
          BillSimulationProductModel.decode(
              getBillSimulationProductResult.value!);
      ...
      billSimulationProductsJson =
          BillSimulationProductModel.encode(billSimulationProducts);
    } else {
      // 첫 이용시 추가한 프로덕트 하나만 인코딩
      billSimulationProductsJson =
          BillSimulationProductModel.encode([billSimulationProduct]);
    }
